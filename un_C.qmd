...
To be able to run the next commands, you need to download the file `calcio.csv`
and save it on your computer. [Link to the file](data/calcio.csv).
:::
...
To open the file `calcio.csv`, you must provide **R** with the **path of the
...
The `calcio` dataset therefore contains a total of $7$ variables. Each
...
The variable `Date` of the `calcio` dataset represents a date and must therefore
be coded as such.
...
`calcio`, containing `TRUE` if the value is missing and `FALSE` otherwise.

The function `rowSums(matrix)` produces a vector whose values are the sum of the
elements of each row of the `matrix`.

As a consequence, `rowSums(is.na(calcio))` indicates how many missing values are
present in each row.
